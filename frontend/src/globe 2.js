  ...
  const getSeason = (date) => {
    ...
    if (month >= 2 && month <= 4)
      return {
        ...
        description:
          "Earth awakens. Flowers bloom across continents as warmth returns.",
    ...
    if (month >= 5 && month <= 7)
      return {
        ...
        description:
          "Maximum photosynthesis. The planet pulses with life at its zenith.",
    ...
    return {
      ...
      description:
        "Minimal growth. The planet breathes quietly, waiting for spring.",
  ...
  return (
    <div className="relative w-full h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 overflow-hidden">
      ...
      <div
        ...
        style={{
          background: `radial-gradient(circle, ${
            season.color.includes("pink")
              ? "#ec4899"
              : season.color.includes("green")
              ? "#10b981"
              : season.color.includes("amber")
              ? "#f59e0b"
              : "#3b82f6"
          }, transparent)`,
        }}
      ...
      <div className="absolute top-8 left-8 bg-black/80 backdrop-blur-xl p-8 rounded-3xl border border-white/10 shadow-2xl max-w-md">
        <h1
          className={`text-4xl font-bold bg-gradient-to-r ${season.color} bg-clip-text text-transparent mb-2`}
        >
          Earth's Bloom Pulse
        </h1>
        <p className="text-sm text-gray-400 mb-6">
          Global NDVI Visualization • NASA MODIS
        </p>
        ...
        <div className="space-y-4">
          ...
          <div
            className={`p-4 rounded-2xl bg-gradient-to-r ${season.color} bg-opacity-10 border-l-4 border-emerald-500`}
          >
